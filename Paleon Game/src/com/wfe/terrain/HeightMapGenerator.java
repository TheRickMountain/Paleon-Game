...
private static int size = 256;
	...
	public static BufferedImage generatePerlin(int octaves, float roughness) {
		seed = random.nextInt(1000000000);
		I = new BufferedImage(size, size, BufferedImage.TYPE_INT_RGB);
		
		for(int x = 0; x < size; x++) {
			for(int y = 0; y < size; y++) {
				float color = generatePerlin(x, y, octaves, roughness);
				color *= 1.45f;
	...
	public static BufferedImage generatePerlinIsland(int octaves, float roughness) {
		seed = random.nextInt(1000000000);
		I = new BufferedImage(size, size, BufferedImage.TYPE_INT_RGB);
	...
	private static float generatePerlin(int x, int z, int octaves, float roughness) {
		float total = 0;
